...
- `AI_SERVICE_URL="http://localhost:5000"` — базовый URL Python FastAPI нейросервиса (см. `AI_INTEGRATION.md`). Если не задан или сервис недоступен, сервер продолжит офлайн-логику без падений.

База данных: PostgreSQL (Prisma). Схема и миграции описаны в `prisma/schema.prisma`.

## Эндпоинты

### Анализ товара (`/products`)

Проверка желаемой покупки и расчёт периода охлаждения с безопасной интеграцией внешней нейросети.
...
- Ответ:
  ...
  ```
- Логика:
  - Если категория в чёрном списке пользователя → `BLOCKED`.
  - Дополнительно: вызывается AI-проверка схожести категории с элементами чёрного списка. При высокой схожести (например, > 0.7) категория считается заблокированной, возвращаем `BLOCKED` с причиной от AI.
  - Иначе рассчитывается период охлаждения по диапазонам цены (`CoolingRange`).
  - Если включено `useSavingsCalculation`, добавляется период согласно «половина от месячного объёма».
  - Если дней охлаждения 0 и хватает накоплений → `APPROVED`, иначе → `COOLING`.
  - Затем вызывается AI-совет по покупке: поля `ai_reason` и `ai_advice` заполняются рекомендациями нейросети. Если нейросеть уверенно рекомендует утвердить/заблокировать (статус `APPROVED`/`BLOCKED`), итоговый статус корректируется.

Примечание: AI-вызовы выполняются с таймаутом и безопасным фолбэком — при недоступности нейросервиса сервер возвращает корректный ответ, используя офлайн-логику.
...
Запуск нейросервиса (пример):

```bash
# Python FastAPI сервис должен быть доступен по AI_SERVICE_URL
